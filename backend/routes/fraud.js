...
($1,$2,$3,$4,$5,$6,now()) 
          ...
          [uuidv4(), tid, 'repeat_winner', score, JSON.stringify({ contractor: row.contractor, wins: 
Number(row.wins) }), 'pending'] 
        ); 
      } 
    } 
    ...
    for (const cat of categories) { 
      ...
      for (const o of outliers.rows) { 
        ...
        await pool.query( 
          `INSERT INTO fraud_flags (id,tender_id,rule,score,evidence,status,created_at) VALUES 
($1,$2,$3,$4,$5,$6,now())`, 
          [uuidv4(), o.id, 'price_outlier', score, JSON.stringify({ category: cat, amount: o.amount, mean, 
std, cutoff }), 'pending'] 
        ); 
      } 
    } 
...
beneficiary_id: d.beneficiary_id, count: Number(d.cnt) }), 'pending'] 
        ); 
      } 
    } 
    ...
    for (const n of nodes) { 
      adj[n.contractor_id] = adj[n.contractor_id] || { contractor: n.contractor, contractor_id: 
...
(clusterNodes.length*(clusterNodes.length-1))) : 0)); 
      ...
      clusters.push({ cluster_nodes: clusterNodes, suspiciousness_score, total_amount, edge_density, 
